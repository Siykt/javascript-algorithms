import { listToTreeByEffectiveNode, listToTreeByLevelOrder } from './listToTree'
...
describe('将一个非空的数组转换为一个树: listToTreeByLevelOrder', () => {
  test('测试输入: [1, 2, 3, 4, 5]', () => {
...
describe('将一个非空的数组转换为一个树: listToTreeByEffectiveNode', () => {
  test('测试输入: [1, 2, 3, 4, 5]', () => {
    const list = [1, 2, 3, 4, 5]
    const root = listToTreeByEffectiveNode(list)
  ...
  test('测试输入: [1, null, 2, null, 3, null, 4, null, 5]', () => {
    ...
    expect(levelOrder(root)).toStrictEqual([[1], [2], [3], [4], [5]])
    expect(preorderTraversal(root)).toStrictEqual([1, 2, 3, 4, 5])
    expect(inorderTraversal(root)).toStrictEqual([1, 2, 3, 4, 5])
    expect(postorderTraversal(root)).toStrictEqual([5, 4, 3, 2, 1])
  })
  ...
  test('边界测试, 输入: [1, null, null, 2, 3]', () => {
    const list = [1, null, null, 2, 3]
    const root = listToTreeByEffectiveNode(list)
